...
function pdcs_sz_eval(wduration)
  ...
  logfile = "logs/pdcs_sz_eval_" + wduration + ".txt";
  delete(logfile);
  diary(logfile);
    ...
    for index = 1:folder_info_size
      ...
      for inc = 1:stop_inc
        ...
        if (pred ~= 6)
          ...
          switch pred
              case 8
                ssim = sz_compressive_sensing(x, focal_phi, focal_psi, focal_A);
                ssims_1window = [ssims_1window ssim];
                pdcs_data = pdcs_data + focal_m;
              case 12
                ssim = sz_compressive_sensing(x, absence_phi, absence_psi, absence_A);
                ssims_1window = [ssims_1window ssim];
  ...
  output_ssims_file_prefix = "logs/pdcs_sz_eval_" + wduration;
