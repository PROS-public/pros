...
psi_matrix = "inv_bior6.8_750.csv"; % Focal

test_matrix_folder = "./phi_matrices/";
test_matrix = "Phi_bin_240x750.csv";
...
for index = 2:num_of_test_samples+1
        ...
        start_sample = 1;
        stop_sample = n;
        signal = data(start_sample:stop_sample);
        x = round(signal);

        % Resample to 4000Hz
        [Numer, Denom] = rat(up_fs/fs);
        x_resampled = round(resample(signal, Numer, Denom));
