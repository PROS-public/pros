...
static constexpr int sz_absence_tensor_arena_size = 30 * 1024;
static uint8_t sz_absence_tensor_arena[sz_absence_tensor_arena_size];

/*---------------------------------------------------------------------------------------------*/
sz_absence_pr::sz_absence_pr(void) {
...
/*---------------------------------------------------------------------------------------------*/
void sz_absence_pr::init(tflite::ErrorReporter *global_error_reporter) {
  error_reporter = global_error_reporter;

  // Map the model into a usable data structure. This doesn't involve any
  // copying or parsing, it's a very lightweight operation.
  pr_model = tflite::GetModel(absence_pr);
  if (pr_model->version() != TFLITE_SCHEMA_VERSION) {
    TF_LITE_REPORT_ERROR(error_reporter,
        "sz_absence_pr: Model provided is schema version %d not equal "
  ...
  static tflite::MicroInterpreter static_interpreter(pr_model,
      op_resolver, sz_absence_tensor_arena, sz_absence_tensor_arena_size,
      error_reporter);
  ...
  if (allocate_status != kTfLiteOk) {
    TF_LITE_REPORT_ERROR(error_reporter, "sz_absence_pr: AllocateTensors() failed");
    while(1);
  }
  ...
  if ((model_input->dims->size != 4)
    ...
    TF_LITE_REPORT_ERROR(error_reporter,
        "sz_absence_pr: Bad input tensor parameters in model");
    while(1);
  }
...
/*---------------------------------------------------------------------------------------------*/
float32_t sz_absence_pr::recognise(float32_t *input_p, uint32_t input_buffer_size) {
