 ...
 * @param      scaling     signal scaling factor
 */
void buf_to_float(int16_t *pInSignal, float32_t *pOutSignal, uint32_t len, uint32_t scaling)
{
  float32_t temp;

  for (uint32_t i = 0; i < len; i++)
  {
    temp = (float32_t) pInSignal[i];
    pOutSignal[i] = temp/scaling;
  }
}
